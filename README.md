...
### 2026-08-12
- **Pembaruan Fitur Pembuatan Laporan Dinas (LPD)** — Menambahkan field filter **Pegawai / Petugas** pada form LPD khusus Super Admin yang secara **default otomatis terisi akun pengguna yang sedang login** (sehingga Surat Tugas milik pengguna sendiri langsung muncul tanpa tergeser/terpotong oleh data ribuan ST pegawai lain). Super Admin juga dapat mengganti filter ke pegawai lain secara dinamis. Menyesuaikan filter pilihan Surat Tugas sehingga hanya menampilkan Surat Tugas yang **belum dibuatkan LPD** serta mengecualikan petugas kolektif/mitra ("Terlampir"). Menambahkan tampilan tanggal/periode tugas (contoh: `(12 - 14 Agu 2026)`) pada label pilihan Surat Tugas di dropdown, serta dukungan auto-fill dan tombol aksi **Buat LPD** / **Lihat LPD** langsung di tabel Surat Tugas.
...
- **Sinkronisasi Otomatis Form SPPD** — Nomor SPPD dan Nomor Urut SPPD kini tersinkronisasi 2 arah secara *real-time* di dalam form UI. Jika Anda mengubah format `B-20/...`, maka *field* Nomor Urut akan otomatis menjadi `20`, dan sebaliknya. Penomoran SPPD kembali menggunakan urutan mandirinya sendiri, tidak meminjam nomor urut dari Surat Tugas.
...
### 2026-04-09
- **Penyempurnaan Form dan View Survey** — Form pembuatan Survey sekarang secara *default* langsung memiliki status Aktif / *is_active* bernilai true. Selain itu, *Tab Filter* berbasis peran (Role) petugas kini ditambahkan pada bagian Peserta/Petugas Survey maupun di menu utama *Manajemen Pengguna*. Tersedia tab untuk "Semua", "Mitra", "Pegawai", dan "Ketua Tim" agar proses pencarian lebih mudah.
...
- **Monitor Nomor Surat Detail** — Widget Monitor Nomor Surat di dashboard/list sekarang memisahkan antara "Nomor Belum Terpakai / Terlewat" dan "Nomor Di-block (Khusus / Reserved)". Nomor yang di-block juga dikelompokkan berdasarkan nama kegiatannya (keterangan) agar lebih mudah dibaca. Tersedia juga tombol "Kelola & Buat ST" langsung pada widget.
...
### 2026-07-21
- **Solusi Final Error 401 Pulse Analytics di Server Production** — Mengatasi masalah gate authorization bawaan Pulse yang menimpa gate kustom dengan memindahkan deklarasi Gate `viewPulse` ke dalam fungsi `app()->booted()` di `AppServiceProvider`. Menggunakan top-level HTTP redirect (`navigate: false`) pada halaman `PulseAnalytics` agar cookie sesi aman terkirim, serta menggunakan middleware `auth:web` pada konfigurasi `pulse.php`.
